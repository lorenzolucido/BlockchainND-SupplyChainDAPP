...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier checkValue(uint _upc) {
    ...
    //address payable _to = address(uint160(_make_payable(items[_upc].consumerID));
  ...
  function kill() public onlyOwner() {
    if (msg.sender == getOwner()) {
      selfdestruct(_make_payable(getOwner()));
    }
  }

  // Define a function 'harvestItem' that allows a farmer to mark an item 'Harvested'
  function harvestItem(uint _upc, address _originFarmerID, string memory originFarmName, string memory originFarmInformation, string memory _originFarmLatitude, string memory _originFarmLongitude, string memory _productNotes) public onlyFarmer verifyCaller(_originFarmerID)
  {
    ...
    item.originFarmInformation = originFarmInformation;
    item.originFarmName = originFarmName;
  ...
  function fetchItemBufferTwo(uint _upc) public view returns (uint itemSKU, uint itemUPC, uint productID, string memory productNotes, uint productPrice, uint itemState, address distributorID, address retailerID, address consumerID) {
    ...
    Item memory item = items[_upc]; 
    //uint itemState = 99;
    if(item.itemState == State.Harvested) itemState = 0;
    if(item.itemState == State.Processed) itemState = 1;
    if(item.itemState == State.Packed) itemState = 2;
    if(item.itemState == State.ForSale) itemState = 3;
    if(item.itemState == State.Sold) itemState = 4;
    if(item.itemState == State.Shipped) itemState = 5;
    if(item.itemState == State.Received) itemState = 6;
    if(item.itemState == State.Purchased) itemState = 7;

    return (item.sku, item.upc, item.productID, item.productNotes, item.productPrice, itemState, item.distributorID, item.retailerID, item.consumerID);
  }
}
